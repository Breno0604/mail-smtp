...
Manter o `<span class="text-red-600">*</span>` — isso é tratado pelo renderizador.
...
- `showSection()`: Chamar `renderIniciais()` quando `n === 1` (para recriar campos
  após reset, aligned com as demais seções que já fazem isso).
...
- **Retorno sem tipo selecionado**: Exibir placeholder pedindo para selecionar tipo,
  ou renderizar "Descrição" mesmo sem tipo. *Decisão: renderizar sempre o campo
  "Descrição", com aviso sutil se não houver tipo selecionado.*
