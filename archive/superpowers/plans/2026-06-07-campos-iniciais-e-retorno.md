...
function restoreSavedState() {
  ...
  try {
    ...
    const ok = confirm(
      `H\u00E1 um formul\u00E1rio salvo (Se\u00E7\u00E3o ${section} de 5). Deseja continuar de onde parou?`
    );
    if (!ok) {
...
export function renderRetorno() {
  ...
  retornoFields.forEach(field => {
    ...
    input.placeholder = field.label;
    input.className =
      'w-full px-3 py-2.5 border border-gray-300 rounded-lg text-base font-sans text-gray-900 bg-white outline-none transition-all resize-y min-h-[80px] focus:border-blue-600 focus:ring-3 focus:ring-blue-600/15';
